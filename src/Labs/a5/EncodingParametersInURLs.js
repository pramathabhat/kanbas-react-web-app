...
function EncodingParametersInURLs() {
  ...
  const fetchSubtraction = async (a, b) => {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/a5/subtract/${a}/${b}`);
    setResult(response.data);
  };
  ...
  return (
    <div>
      ...
      <a href={`${process.env.REACT_APP_BASE_URL}/a5/add/${a}/${b}`} className="btn btn-primary">
        Add {a} + {b}
      </a>
      <a
